...
var Timeline = function($container, params) {
    ...
    var cloud_cover_range = function(obj) {
      ...
      else if(obj.cloudCover < 0.93750) {
    ...
    * best-fit to some NOAA data.) */
    var snowToWaterRatio = function(t) {
      var r
      if(t) {
        r = Math.max(8.0, Math.min(75.0,
          (0.014666443251269316 * t + -1.4349642980102439) * t + 41.593227138990002
        ))
      } else {
        r = 10.0
      }
    ...
    var inPerHourToEstimatedDBZ = function(inches_per_hour) {
      return (inches_per_hour <= 0) ? 0 : 10.0 * Math.log10(
                  200 * Math.pow(25.4 * inches_per_hour, 1.6));
    }
    ...
    var precip_range = function(obj) {
      var min_intensity = (params.units == 'us') ? 0.005 : .127
    ...
    var create_hour_ticks = function(hourly) {
      ...
      for(var i = 0; i < hourly.length; i++) {
        ...
        $('<span />').addClass(i % 2 == 0 ? 'even' : 'odd').css(params.reversed ? 'right' : 'left', x).appendTo($ticks)
        x += spacing
      }
    ...
    var create_hours = function(hourly) {
      ...
      for(var i = 0; i < hourly.length; i++) {
        ...
        if(left >= 0 && left < timeline_width) {
          var $hour = $('<span />').addClass('hour').css(params.reversed ? 'right' : 'left', left)
          ...
          if(params.event === true && hourly.length < 10) {
            if(i == 0 && params.now) {
              ...
              var span = $('<span />').html(hr).addClass(hr.replace(' ', '')).appendTo($hour)
          }
          else {
            if(i == 0 || (i > 1 && i % 2 == 0)) {
              if(i == 0 && params.now) {
      ...
      $hours.find('.hour').eq(0).addClass('first')
    }
    ...
    var create_stripes = function(stripes) {
      ...
      for(var i = 0; i < stripes.length; i++) {
        ...
        var $span = $('<span />').addClass("timeline-stripe-" + stripes[i].condition)
    ...
    var create_temperatures = function(hourly) {
      ...
      if(params.event === true) {
        let icon_container = $("<div />").addClass("icons").appendTo($(".timeline"));
      }
      ...
      for(var i = 0; i < hourly.length; i++) {
        if(hourly[i].temperature !== null) {
          ...
          if(hourly[i].temperature < min_temp)
            min_temp = hourly[i].temperature
  
          if(hourly[i].temperature > max_temp)
            max_temp = hourly[i].temperature
        }
      }
      ...
      var add_temp = function(i) {
        ...
        if(icon) {
          var $icon = $("<span class=\"skycon icon\" style=\"width:25px;height:25px\"><canvas width=\"64\" height=\"64\" style=\"width:100%;height:100%\" class=" + icon + "></canvas></span>").css('opacity', opacity)
        }
  
        var $elem = $('<span><span>'+temp+'</span></span>').css('opacity', opacity)
        if(params.reversed) {
          $elem.css('right', left)
          if(icon) {
            if(i === 0) {
    ...
    var add_icons = function(hourly) {
      var $icon_container = $("<div />").addClass("icon-container").css("width", timeline_width);
    ...
    var add_last_unit = function(eventLastHour) {
      ...
      // Add tick
      var class_name = ($(".hour_ticks span:last-child").hasClass("even")) ? "odd" : "even";
      var tick = $("<span />").addClass(class_name).css("left", timeline_width - 2 + "px");
      $(".hour_ticks").append(tick);
      // Add time
      var time = hour_str(eventLastHour.time);
      var $hour = $('<span />').addClass('hour').css(params.reversed ? 'right' : 'left', timeline_width - 20 + "px");
      var span = $('<span />').html(time).addClass(time.replace(' ', '')).appendTo($hour);
      $hours.append($hour);
      // add temp
      ...
      var $icon = $("<span class=\"skycon icon\" style=\"width:25px;height:25px\"><canvas width=\"64\" height=\"64\" style=\"width:100%;height:100%\" class=" + icon + "></canvas></span>")
    ...
    timeline.load = function(hours, tz_offset, currently) {
      ...
      timezone_offset = (tz_offset || 0) + new Date(1000 * start_time).getTimezoneOffset() / 60
  
      var hourly = [], eventLastHour
        ...
        for(var i = 0; i < hourly.length; i++) {
          if(Math.abs(hourly[i].time - currently.time) <= 1800) {
            for(var prop in hourly[i]) {
              ...
              hourly[i][prop] = (currently[prop] != null) ? currently[prop] : hourly[i][prop]
            }
            break
          }
        }
      ...
      if(params.width) {
        ...
        var min_hour_spacing = Math.ceil(params.label_width / single_hour_spacing - 0.0001) // subtract delta because of stupid float imprecision
      ...
      for(var i = 0; i < hourly.length; i++) {
        ...
        let precipType = (hourly[i].precipType != null) ? hourly[i].precipType : "rain";
        ...
        let isPrecip = (icon === "rain" || icon === "snow" || icon === "sleet") ? true : false;
